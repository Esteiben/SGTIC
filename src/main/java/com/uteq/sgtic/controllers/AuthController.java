...
public class AuthController {
    ...
    public ResponseEntity<LoginResponseDTO> login(@RequestBody LoginRequestDTO request, HttpServletRequest httpRequest) {
        ...
        userRepository.findByEmail(request.getEmail()).ifPresent(user -> {
            ...
            String sql = "INSERT INTO public.audit_login (id_usuario, exitoso, ip_address, user_agent) VALUES (?, true, cast(? as inet), ?)";
            try {
                jdbcTemplate.update(sql, user.getId(), ipAddress, finalUserAgent);
